...
namespace LawEditor.Services.WordServises {
    public class WordFileProsesingService {
        ...
        // ИСПРАВЛЕНО: итерируем по всем дочерним элементам каждого Run,
        // чтобы подобрать все <w:t> и <w:br> внутри одного Run.
        // Раньше использовался GetFirstChild<Text>() — он брал только первый
        // <w:t>, пропуская остальные (например, в параграфе с подписью всё
        // содержимое лежит в одном Run с несколькими <w:t> и <w:br> между ними).
        private string GetParagraphText(Paragraph para) {
            ...
            foreach (var run in para.Elements<Run>()) {
                ...
                // Проверяем field state через первый FieldChar в run
                var fldChar = run.Descendants<FieldChar>().FirstOrDefault();
                if (fldChar != null) {
                ...
                // ── Итерируем по всем дочерним элементам Run ─────────────────
                // Это позволяет собрать текст из нескольких <w:t> внутри одного
                // Run, разделённых <w:br> (как в параграфе с подписью президента).
                // Важно: в некоторых файлах <w:t> уже содержит \n в конце,
                // а за ним ещё идёт <w:br/> — это даёт двойной перенос.
                // Решение: из текста <w:t> убираем \n, перенос добавляет только <w:br/>.
                foreach (var child in run.ChildElements) {
                    if (child is Text textNode) {
                        // Убираем \n из текста узла — переносы строк обрабатывает <w:br/>
                        string text = (textNode.Text ?? "").Replace("\n", "").Replace("\r", "");

                        if (isSuperscript && prevText != null &&
                            prevText.Length > 0 && prevText[prevText.Length - 1] is >= '0' and <= '9')
                            sb.Append('.');
                        ...
                        // <w:br/> — единственный источник переноса строки
                        sb.Append('\n');
                    }
                }
            }
        ...
        public Laws ReadWordFile(string filePath) {
            ...
            // ── Глобальный флаг подписи ───────────────────────────────────────
            // Как только в любом параграфе найден İlham ƏLİYEV — весь
            // оставшийся текст документа идёт в TransitionalProvisions.Date.
            // Весь блок подписи может лежать в одном параграфе с <w:br> внутри
            // одного Run; GetParagraphText теперь корректно разворачивает их.
            ...
            foreach (var element in body.ChildElements) {
                ...
                // Получаем полный текст параграфа (включая <w:br> как \n)
                var line = GetParagraphText(para);

                // ════════════════════════════════════════════════════════════
                // ГЛОБАЛЬНАЯ ПРОВЕРКА ПОДПИСИ
                // Проверяем до любой другой логики, в любом режиме.
                // Используем Contains по всему тексту параграфа — это важно,
                // потому что весь блок подписи может быть одним параграфом.
                // ════════════════════════════════════════════════════════════
                if (!signatureDetected &&
                    line.Contains("İlham ƏLİYEV", StringComparison.OrdinalIgnoreCase)) {
            ...
            // ── Сохраняем накопленную подпись в TransitionalProvisions.Date ───
            if (signatureDetected && signatureBuilder.Length > 0) {
                // Схлопываем множественные переносы строк в одиночные
                string raw = signatureBuilder.ToString().TrimEnd();
                TransitionalProvisions.Date = Regex.Replace(raw, @"\n{2,}", "\n");
            }
